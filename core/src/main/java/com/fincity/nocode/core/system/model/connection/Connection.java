...
public class Connection implements Serializable {
	...
	public static final Schema SCHEMA = new Schema().setType(new SingleType(OBJECT))
	        ...
	        .setName(SCHEMA_NAME)
	        .setVersion(1)
	        .setProperties(Map.of("id", Schema.ofString("id"), "type", Schema.of("type", STRING)
	                .setEnums(Stream.of(ConnectionType.values())
	                        .map(Object::toString)
	                        .map(JsonPrimitive::new)
	                        .map(JsonElement.class::cast)
	                        .toList()),
	                PROPS_STRING, Schema.of(PROPS_STRING, OBJECT)
	                        .setAnyOf(List.of(MongoDBCProperties.SCHEMA, R2DBCProperties.SCHEMA))));
	...
	public static Connection from(JsonObject jsonObject) {
		...
		ConnectionType cType = ConnectionType.valueOf(jsonObject.get("type")
		        .getAsString());
